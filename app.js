(() => {
  ...
  const colors = [['#edf1ff','#4963ae'],['#edf7f3','#2a7f60'],['#fff3e4','#916622'],['#f3edff','#815aa9'],['#fceef4','#a45d7c']];
  const icons = new Set(['book','clock','check','edit','trash','flag','circle-check']);
  let toastTimer;
  ...
  function load() {
    try { state.tasks = readSaved(); $('storage-error').hidden = true; }
    catch(error) { reportStorage(error); }
  }
  function commit(transform) {
    const current = readSaved(); // Read the latest version so changes in another tab are preserved.
  ...
  function renderTask(task) {
    ...
    checkbox.addEventListener('change',() => {
      ...
      } catch(error) { checkbox.checked = task.completed; showError(error); }
    });
    ...
    const palette = colors[M.SUBJECTS.indexOf(task.subject) % colors.length];
    subject.style.setProperty('--subject-bg',palette[0]); subject.style.setProperty('--subject-color',palette[1]);
    ...
    meta.append(subject,dueEl);
    if (task.priority === 'high') {
      const priority = node('span','priority-tag'); priority.append(icon('flag'),node('span','','Чухал')); meta.append(priority);
    }
    content.append(meta);
    if(task.notes) content.append(node('p','task-notes',task.notes));
    const actions = node('div','task-actions');
    actions.append(actionButton('edit',task.title + ' — засах',() => openTask(task.id)),actionButton('trash',task.title + ' — устгах',() => openDelete(task.id),true));
  ...
  function renderCalendar() {
    ...
    ['Да','Мя','Лх','Пү','Ба','Бя','Ня'].forEach((label,index) => {
      ...
      cell.append(node('span','',label),node('strong','',day.getDate())); strip.append(cell);
    });
  ...
  function render() {
    ...
    $('progress-percent').textContent = percent + '%'; $('progress-bar').value = percent;
    ...
    if(state.subject !== 'all') {
      ...
      $('empty-description').textContent = 'Хичээл, хийх зүйл, дуусгах өдрөө бичээд эхлээрэй.';
    }
    const next = M.select(state.tasks,'all','all',state.today)[0];
    const nextContainer = $('next-task'); nextContainer.replaceChildren();
    if(next) {
      const info = dueInfo(next);
      const due = node('div','next-due ' + info.className); due.append(icon('clock'),node('span','',info.label));
      nextContainer.append(node('p','next-subject',next.subject),node('h3','',next.title),due);
    } else nextContainer.append(node('h3','','Одоогоор төлөвлөөгүй'),node('p','','Даалгавар нэмэхэд хамгийн ойр хугацаа энд харагдана.'));
  }
  function setView(view) { state.view = view; state.subject = 'all'; $('subject-filter').value = 'all'; render(); }
  function openTask(id = null) {
    load();
    const task = state.tasks.find(t => t.id === id);
    if(id && !task) { render(); notify('Энэ даалгавар өөр цонхонд устсан байна.'); return; }
    $('task-form').reset(); $('form-error').hidden = true;
    $('task-id').value = task ? task.id : '';
    $('dialog-title').textContent = task ? 'Даалгавар засах' : 'Даалгавар нэмэх';
    if(task) {
      for(const key of ['subject','title','due','priority','notes']) $('task-' + key).value = task[key];
    } else {
  ...
  function saveFromForm(event) {
    ...
    const saveButton = $('save-task'); saveButton.disabled = true;
    try {
      const input = {};
      for (const key of ['subject','title','due','priority','notes']) input[key] = $('task-' + key).value;
      ...
      commit(tasks => {
        ...
        const newId = window.crypto && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : Date.now().toString(36) + '-' + Math.random().toString(36).slice(2);
  ...
  M.SUBJECTS.forEach(subject => {
    for (const id of ['task-subject','subject-filter']) { const option = node('option','',subject); option.value = subject; $(id).append(option); }
  });
  ...
  $('confirm-delete').addEventListener('click',() => {
    try {
      let removed;
      commit(tasks => { removed = tasks.find(t => t.id === state.deleting); return tasks.filter(t => t.id !== state.deleting); });
      $('delete-dialog').close(); state.deleting = null;
  ...
  $('undo-button').addEventListener('click',() => {
    ...
    try {
      ...
      setView(task.completed ? 'done' : 'all'); notify('Даалгаврыг сэргээв.');
    } catch(error) { showError(error); }
  });
  $('dismiss-toast').addEventListener('click',() => {clearTimeout(toastTimer); $('toast').hidden = true; state.undo = null;});
  window.addEventListener('storage',event => {if(event.key === M.KEY || event.key === null) {load(); render();}});
  document.addEventListener('visibilitychange',() => {if(!document.hidden) {load(); render();}});
  setInterval(() => {if(M.localDate() !== state.today) render();},60000);
  load(); render();
  ...
  if(context && typeof context.registerTool === 'function') {
    ...
    const register = tool => {
      ...
      catch (_) { /* The standard is optional; the visible app stays available. */ }
    };
    register({name:'list_homework',title:'Даалгавруудыг харах',description:'Read homework stored in this browser. Returns the current list without making changes.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:true},execute(input){if(!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).length) throw new Error('Expected an empty object.');load();render();return {tasks:state.tasks.map(t => ({...t})),storageReadable:$('storage-error').hidden};}});
    register({name:'start_homework_creation',title:'Даалгавар нэмэх цонх нээх',description:'Open the homework creation form. This does not save a task; the user completes and saves the visible form.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute(input){if(!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).length) throw new Error('Expected an empty object.');if($('delete-dialog').open) $('delete-dialog').close();if(!$('task-dialog').open) openTask();return {formOpen:true,saved:false};}});
